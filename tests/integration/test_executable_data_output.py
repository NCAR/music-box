...
def test_create_netcdf_with_timestamp(temp_dir):
    # the only way to create a NetCDF file is to provide a filename with extension
    run_main_with_args(['-e', 'Analytical'], temp_dir)
    assert (not glob.glob(os.path.join(temp_dir, "music_box_*.nc")))
...
def test_create_named_csv(temp_dir):
    run_main_with_args(['-e', 'Analytical', '-o', 'out.csv'], temp_dir)
    assert os.path.exists(os.path.join(temp_dir, "out.csv"))


def test_create_named_netcdf(temp_dir):
    run_main_with_args(['-e', 'Analytical', '-o', 'out.nc'], temp_dir)
...
def test_create_directory_and_timestamped_netcdf(temp_dir):
    ...
    os.makedirs(os.path.join(temp_dir, "results"), exist_ok=True)
    run_main_with_args(['-e', 'Analytical', '-o', 'results/'], temp_dir)
    assert (not glob.glob(os.path.join(temp_dir, "results/music_box_*.nc")))
