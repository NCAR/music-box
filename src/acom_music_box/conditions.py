...
class Conditions:
    ...
    def add_species_concentration(self, species_concentration):
        """
        Add a species concentration to the list of species concentrations.

        Args:
            species_concentration (tuple[str, float]): The species name and concentration to be added.
        """
        self.species_concentrations.append(species_concentration)

    def add_rate_parameter(self, rate_parameter):
        """
        Add a user-defined rate parameter to the list of user-defined rate parameters.

        Args:
            rate_parameter (tuple[str, float]): The rate parameter to be added.
        """
        self.rate_parameters.append(rate_parameter)

    def get_concentrations(self):
        """
        Retrieves a dictionary of concentrations from the species_concentrations list.

        Returns:
            dict: A dictionary containing concentrations of each species.
        """
        concentrations = {}
        for name, concentration in self.species_concentrations:
            concentrations[name] = concentration

        return concentrations

    def get_user_defined_rate_parameters(self):
        """
        Retrieves a dictionary of user-defined reaction rate parameters.

        Returns:
            list: A dictionary containing all user-defined reaction rate parameters.

        """
        rate_parameters = {}
        for name, parameter in self.rate_parameters:
            rate_parameters[name] = parameter
